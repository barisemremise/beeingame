...
def insert_db():
    ...
    companies='''INSERT INTO company(company_name, ceo_name, year, country, employee)
    VALUES ('Riot Games','Nicolo Laurent',2006,'U.S.',2500),
    ('PUBG Corporation', 'Changhan Kim' ,2017,'South Korea',795),
    ('CD PROJEKT RED','Adam Kiciński',1994,'Poland',1111),
    ('Innersloth','Marcus Bromander',2015,'U.S.',4),
    ('SCS Software','Pavel Šebor',1997,'Czechia',170),
    ('Electronic Arts','Andrew Wilson',1982,'U.S.',9300),
    ('Rockstar Games','Sam Houser',1998,'U.S.',2189),
    ('Crytek','Avni Yerli',1999,'Germany',290),
    ('Activision','Bobby Kotick',1979,'U.S.',9200),
    ('Valve','Gabe Newell',1996,'U.S.',360),
    ('NetherRealm Studios','Shaun Himmerick',2010,'U.S.',235),
    ('2K','Strauss Zelnick',2005,'U.S.',1217),
    ('Rocksteady Studios','Sefton Hill',2004,'United Kingdom',245),
    ('Red Barrels','Philippe Morin',2011,'Canada',27),
    ('Ubisoft','Yves Guillemot',1986,'France',18045),
    ('Playground Games','Gavin Raeburn',2010,'United Kingdom',200),
    ('Square Enix','Yosuke Matsuda',1975,'Japan',4601),
    ('TaleWorlds Entertainment','Armağan Yavuz',2005,'Turkey',90),
    ('Tantalus Media','Tom Crago',1994,'Australia',27),
    ('BANDAI NAMCO Studios Inc.','Satoshi Oshita',2012,'Japan',934),
    ('Quantic Dream','David Cage',1997,'France',180),
    ('4A Games','Dean Sharpe',2006,'Ukraine',150),
    ('SMG Studio','Ashley Ringrose',2013,'Australia',26),
    ('Codemasters','Frank Sagnier',1986,'United Kingdom',700),
    ('Berserk Games','Jason Henry',2014,'U.S.',5),
    ('Mediatonic','Dave Bailey Paul Croft',2005,'United Kingdom',230),
    ('Mojang AB','Jonas Mårtensson',2009,'Sweden',70),
    ('Blizzard Entertainment','Michael Morhaime',1991,'U.S.',4700),
    ('Infinity Ward','Vince Zampella',2002,'U.S.',444),
    ('CAPCOM','Kenzo Tsujimoto',1979,'Japan',2988),
    ('Psyonix LLC','Dave Hagewood',2000,'U.S.',122);'''
...
def create_gamebase(db):
    ...
    db.clear_games()
    number='''SELECT game_id FROM game ORDER BY likes DESC, dislikes ASC, game_name ASC'''
...
def get_user(username):
    query='''SELECT user_id, username, name, AGE(birth), gender, email, user_password, is_admin FROM users WHERE username=%s'''
...
def create_comment_list(gameid):
    ...
    for i in record:
        ...
        query='''SELECT user_id, username, name, AGE(birth), gender, email, user_password, is_admin FROM users WHERE user_id=%s'''
...
def delete_game(gameid):
    query='''SELECT genres.genre_id FROM game INNER JOIN game_genre_rel ON game.game_id=game_genre_rel.game_id 
    INNER JOIN genres ON genres.genre_id=game_genre_rel.genre_id WHERE game.game_id=%s'''
    cursor.execute(query,(gameid,))
...
def company_control(company):
    query='''SELECT COUNT(game_id) AS Numberofgames FROM game INNER JOIN company ON company.company_id=game.company_id  WHERE company.company_name=%s GROUP BY company.company_name'''
...
def genre_control():
    query='''DELETE FROM genres USING game_genre_rel WHERE genres.genre_id NOT IN (SELECT genre_id FROM game_genre_rel)'''
    cursor.execute(query)
    connection.commit()

def create_complist():
    query='''SELECT company_id FROM company ORDER BY company_name'''
    cursor.execute(query)
    comps=cursor.fetchall()
    complist=[]
    for i in comps:
        complist.append(create_company(i[0]))
...
def create_company(id):
    ...
    cursor.execute(query,(id,))
    record=cursor.fetchone()
    company=Company(record[0], record[1], record[2], record[3], record[4], record[5])

    return company

def create_gamelist_company(id):
    query='''SELECT game_id FROM game WHERE company_id=%s ORDER BY likes DESC, dislikes ASC, game_name ASC'''
    cursor.execute(query,(id,))
    record = cursor.fetchall()
    games=[]
    for i in record:
        games.append(create_game(i[0]))
    return games

def update_admin(username,update):
